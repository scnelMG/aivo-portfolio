...
async def main() -> None:
    ...
    event = AiEvent(
        ...
        workerType=WorkerType.LLM,
        taskType=TaskType.AUDIENCE_QUESTION_GENERATION,
        status=EventStatus.REQUESTED,
        schemaVersion=1,
        occurredAt=utc_now(),
        payload={
            "presentationId": 100,
            "transcriptUrl": "s3://aivo/transcripts/100.json",
            "slideContentUrl": "s3://aivo/slides/100.json",
            "questionCount": 5,
        },
    )
    ...
    try:
        await client.connect("aivo-llm-test-publisher")
        await client.declare_topology()
        await client.publish_event(event, routing_key)
        logging.info("LLM test event published: eventId=%s routingKey=%s", event.eventId, routing_key)
    finally:
        await client.close()
