...
[담당 AI 구현](#ai-구현-및-의사결정) · [GPU 실행 결과](#ai-음성-분석-구현) · [사용자 피드백](#외부-유저-테스트) · [공개 코드·실행 범위](#공개-소스-범위)
...
박민규가 직접 라벨링한 자료를 기준으로 평가한 결과를 [최종 발표자료](https://ssafy-pjt-presentation-source.vercel.app/)의 기술 소개에서 제시했습니다.

| 발표 평가 항목 | 비교 모델 | aivo | 변화 |
| --- | ---: | ---: | ---: |
| 탐지 정확도 | 모델 A: 5.5% | 76% | +70.5%p |
| 발화 분석 시간 | 모델 B: 41.5초 | 4.3초 | 약 9.6배 빠름 |

발표 뷰어의 22번 화면(탐지 정확도), 23–24번 화면(발화 분석 시간)에서 확인할 수 있습니다. `O` 목록에서는 원본 슬라이드 30–32번입니다. 지표명은 발표자료의 표기를 따르며, 이를 별도의 precision·recall·F1로 바꾸어 해석하지 않습니다.

아래 GPU 스모크 실행은 별도의 실험 기록입니다. 발표자료의 **발화 분석 시간 4.3초**와 아래 **전사 시간 8.731초**는 서로 다른 측정 기록이므로 같은 실행의 수치로 합치지 않습니다.
...
> 모델 선택 기준은 실서비스 파이프라인의 실행 가능성, 단어 시각 근거, 자원 측정 가능성입니다. 위 스모크·탐색 기록과 직접 라벨링한 최종 발표 평가를 구분하며, 발표자료에서 익명화한 모델 A·B를 특정 모델 이름에 임의로 대응시키지 않습니다.
...
현재 공개 코드의 회귀 검사는 [Coaching tests](https://github.com/scnelMG/aivo-portfolio/actions/workflows/coaching-tests.yml)에서 커밋별로 확인할 수 있습니다. 필러 병합·중복 제거·재전사 창 시각·침묵 구간·리뷰 집계를 CPU에서 검사하며, 실험 당시 `27 passed`와는 별도입니다. 실행 방법은 [공개 소스 스냅샷](SOURCE_SNAPSHOT.md#공개-코드-회귀-테스트)에 있습니다.
